...
| `MAX_LOSS_PCT` | -7.0 | **하드 스탑 최대 허용 손실률 (%)** |
| `ENTRY_Z_SCORE_SWING/SHORT` | 2.5 / 3.0 | 진입 Z-Score 임계치 (스윙 24h / 단기 12h) |
| `STOP_LOSS_Z_SCORE` | 4.5 | **통계적 디커플링 손절 임계치** (잔파도 무시용) |
| `CORR_WINDOW_MIN` | 60 | **스마트 손절** 수익률 상관계수 측정 단위 (분) |
| `CORR_STOP_THRESHOLD`| -0.25 | **스마트 손절** 발동 임계치 (1분 수익률 상관 기준) |
...
| 우선순위 | 조건 | 실행 내용 |
|:---:|:---|:---|
| **1** | **Net PnL ≥ +3.5%** | **목표 수익 익절:** 즉각적인 익절 처리 (Maker-Chasing → 시장가 Fallback) |
| **2** | **수익률 상관계수 ≤ -0.25** | **스마트 손절:** 최근 60분 1분봉 "수익률" 상관이 뚜렷한 역방향이면 즉각 꼬리 끊기 |
| **3** | **Net PnL ≤ -7.0%** | **하드 스탑:** Z-Score 도달 여부와 무관하게 순손실이 -7%를 넘어가면 무조건 손절 |
| **4** | **abs(Z-Score) ≥ 4.5** | **통계 손절:** 통계적으로 극단적인 디커플링 진입 시 손절 |
| **5** | **Z-Score 회귀 (EXIT)** | **회귀 익절:** Z-Score가 0을 향해 완전히 관통하고 Net PnL이 양수일 경우 익절 |

> 청산 시 **양 레그 동행 보장**: 한쪽 레그가 체결되면 반대쪽은 즉시 시장가로 따라 닫고,
> 추적 포기(Hold)는 양쪽 모두 미체결로 헷지가 살아있을 때만 허용됩니다.
> 실현 PnL에는 보유 기간의 **펀딩비**가 합산됩니다.
